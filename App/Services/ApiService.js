...
export const ORIGIN_API = isProd ? 'https://quick-app-backend.herokuapp.com' : `http://${IP}:1337`;
...
class ApiService {
    ...
    async getChainPrizesByUser(id) {
        const response = await this.api.get(`chain-prizes?filters[user][id][$eq]=${id}`);
        return response;
    }
    ...
    async paypalWithdrawal(password, amount, email) {
        const response = await this.api.post('withdrawal', { password, amount, email });
        return response
    }

    async getAllTopChains(page) {
        const response = await this.api.get(`/chain-prize/all-top-chains?page=${page}&pageSize=10`)
        return response
    }

    async getAllTopPlayers(page) {
        const response = await this.api.get(`/prize/top-all-players?page=${page}&pageSize=10`)
        return response
    }
