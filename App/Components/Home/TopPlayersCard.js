...
const TopPlayersCard = ({ topPlayers }) => {
  ...
  const renderTopPlayerRow = (
    { name, avatar, totalPrizes, username_ },
    key
  ) => {
    return (
      <View style={styles.topPlayerRow} key={key}>
        <View style={styles.avatarUsernameContainer}>
          <UserAvatar
            size={36}
            url={avatar?.url}
            title={username_?.charAt(0)}
          />
          <Text style={{ fontSize: 16 }}>{username_}</Text>
        </View>
        <View>
          <Text style={styles.addedAmount}>{`${LocaleFormatNumber(
            totalPrizes
          )}€`}</Text>
        </View>
      </View>
  ...
  return (
    ...
    >
      <Card>
        ...
        {topPlayers.map(
          (topPlayer, key) => key < 3 && renderTopPlayerRow(topPlayer, key)
        )}
      </Card>
    </TouchableOpacity>
  );
