...
const TopChainsCard = ({topChains}) => {
  ...
  const renderChainRow = ({ name, acumulated, avatar, username_ }, key) => (
    <View style={stylesTopPlayers.topPlayerRow} key={key}>
      ...
      <View>
        <Text style={stylesTopPlayers.addedAmount}>{`${LocaleFormatNumber(acumulated)}€`}</Text>
      </View>
    </View>
  );
  ...
  return (
    <View style={{ width: '100%'}}>
      <View style={{ width: '100%', alignItems: 'center' }}>
        <Card>
          <TouchableOpacity
            ...
            <View >
              {topChains.map((chain, key) => key < 3 && renderChainRow({ ...chain }, key))}
            </View>
          </TouchableOpacity>
        </Card>
      </View>
      ...
      <View style={{ width: '100%', alignItems: 'center',}}>
        <Card>
        <TouchableOpacity onPress={() => { navigation.navigate('ChainSimulator', { topRanking: topChains, isTopChains: true }) }}>
          <View style={styles.chainSimulatorContainer}>
            <Text style={styles.titleSimulatorChain}>{translate('general.chainSimulator')}</Text>
              <View style={styles.imageContainer}>
                <Image
                  style={styles.gameImage}
                  source={I18n.locale === 'es' ? require('../../Assets/Images/simulateChain2.png') : require('../../Assets/Images/chainSimulator.png')}
                />
              </View>
          </View>
        </TouchableOpacity>
