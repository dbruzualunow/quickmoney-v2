...
export const AuthenticationContextProvider = ({ children }) => {
    ...
    () => ({
      signIn: async (data) => {
        ...
        try {
          ...
          if (response?.data?.jwt) {
            ...
            await ApiService.createOnlineUser({
      ...
      signUp: async (data) => {
        ...
        try {
          ...
          if (
            error?.response?.data?.error?.message === "Email is already taken"
          ) {
            result.errorMsg = translate("authentication.emailAlreadyTaken");
          } else if (
            error?.response?.data?.error?.message === "Username required"
          ) {
            ...
            result.errorMsg = translate(
              "authentication.usernameLengthErrorMin"
            );
          } else if (
            error?.response?.data?.error?.message ===
            "Username length must be less than 50"
          ) {
            result.errorMsg = translate(
              "authentication.usernameLengthErrorMax"
            );
          } else {
  ...
  return (
    <AuthenticationContext.Provider
      value={{ ...authContext, user, refreshUser, state }}
    >
      {children}
    </AuthenticationContext.Provider>
  );
