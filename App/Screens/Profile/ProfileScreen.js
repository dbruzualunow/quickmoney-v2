...
const ProfileScreen = () => {
    ...
    const [isVisibleWithdrawMoneyModal, setIsVisibleWithdrawMoneyModal] = useState(false)
    const [isVisibleDeleteAccountModal, setIsVisibleDeleteAccountModal] = useState(false)
    ...
    useEffect(() => {
        ...
        ApiService.getPrizeSummary()
            .then((response) => {
                const {totalAvailable, numbersPrizesWins, totalPrizes} = response.data
    ...
    const shareInvitationCode = async () => {
        ...
        try {
            const result = await Share.share({
                ...
                message: translate('profile.shareCode', { sharingCode: await buildShareLink(user?.sharingCode) })
            });
    ...
    const foundedCountry = user && countryItems.find(({ value }) => value === user.country)
    ...
    return (
        <View>
                ...
                <View style={styles.mainContainer}>
                    <View style={styles.avatarRowContainerStyle}>
                        ...
                        <View>
                            ...
                            >
                                {user?.username_.length > 15 ? user?.username_.substring(0, 12) + '...' : user?.username_}
                            </Text>
                            <View style={styles.rowIcon}>
                                <Image resizeMode='contain' style={styles.avatarIcon} source={require('../../Assets/Icons/QuickAppIcon.png')} />
                                <Text>
                                    {`${LocaleFormatNumber(totalPrizes)}€`} {translate('general.accumulated')}
                                </Text>
                            </View>
                            <View style={styles.rowIcon}>
                                <Image resizeMode='contain' style={{...styles.avatarIcon, tintColor: "gold",}} source={require('../../Assets/Icons/coin.png')} />
                                <Text>
                                    {`${LocaleFormatNumber(avaliablePrizes)}€`} {translate('general.available')}
                                </Text>
                            </View>
                            <View style={styles.rowIcon}>
                                <Image resizeMode='contain' style={{...styles.avatarIcon, marginLeft: 1}} source={require('../../Assets/Icons/premio.png')} />
                                <Text>
                                    {translate('profile.winnedPrizes', { winnedPrizes: numbersPrizesWins })}
                                </Text>
                            </View>
                            {foundedCountry && 
                            <View style={styles.rowIcon}>
                                <Image resizeMode='contain' style={styles.avatarIcon} source={require('../../Assets/Icons/icon-location.png')} />
                                <Text style={{flex:1, flexWrap: 'wrap'}} >{foundedCountry.label}</Text>
                            </View>}
                        </View>
                    </View>
                    <View style={styles.profileContainer}>
                        ...
                        <TouchableOpacity
                            style={styles.profileRow}
                            onPress={() => { setIsVisibleWithdrawMoneyModal(true) }}
                        >
                        ...
                        <TouchableOpacity
                            style={styles.profileRow}
                            onPress={() => { navigation.navigate('PaymentHistorics') }}
                        >
                        ...
                        <TouchableOpacity
                            style={styles.profileRow}
                            onPress={() => { navigation.navigate('DataPrivacy', { showPrivacy: true }) }}
                        >
                        ...
                        <TouchableOpacity
                            style={styles.profileRow}
                            onPress={() => { navigation.navigate('DataPrivacy', { showTermsAndConditions: true }) }}
                        >
                        ...
                        <TouchableOpacity
                            style={[styles.profileRow, { borderBottomWidth: 0 }]}
                            onPress={() => { setIsVisibleDeleteAccountModal(true) }}
                        >
            ...
            <DeleteAccountModal isVisible={isVisibleDeleteAccountModal} onDismiss={() => { setIsVisibleDeleteAccountModal(false) }} />
            <WithdrawMoneyModal isVisible={isVisibleWithdrawMoneyModal} onDismiss={() => { setIsVisibleWithdrawMoneyModal(false) }} />
            <LogoutModal isVisible={isVisibleLogoutModal} onDismiss={() => { setIsVisibleLogoutModal(false) }} />
