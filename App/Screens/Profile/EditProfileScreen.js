...
const EditProfileScreen = () => {
    ...
    const updateUser = async () => {
        try {
            setLoading(true)
            const response = await ApiService.updateUser(user.id, { name, phoneNumber, email, country, username_ })
            ...
            if (error?.response?.data?.error?.message === 'This attribute must be unique') {
                Alert.alert(
                    "Error",
                    translate('profile.uniqueUsernameError'),
                    [
        ...
        <View>
            ...
            <View style={styles.mainContainer}>
                ...
                <ProfileInput
                    ...
                    value={username_}
                    onChangeText={username_ => { setEditUser({ ...editUser, username_ }) }}
                    maxLength={15}
                />
                <ProfileInput
                    label={translate('general.phone')}
                    value={phoneNumber}
                    onChangeText={phoneNumber => { setEditUser({ ...editUser, phoneNumber }) }}
                />
                <RNPickerSelect
                    placeholder={{ label: translate('authentication.chooseACountry'), value: null }}
                    onValueChange={(value, index) => { setEditUser({ ...editUser, country: value }) }}
                    items={countryItems}
                >
                    <ProfileInput
