...
const ScratchCardScreen = ({ route, navigation }) => {
  ...
  const { winningNumbers, tragaperraIndex, dadoIndex, cardIndex } =
    route.params;
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      for (const key in winningNumbers) {
        if (key.includes("cardNumber")) WC.push(winningNumbers[key]);
        if (key.includes("diceNumber")) WD.push(winningNumbers[key]);
        if (key.includes("slotNumber")) WTP.push(simbolos[winningNumbers[key]]);
      }
