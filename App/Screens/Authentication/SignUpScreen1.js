...
const SignUpScreen1 = (props) => {
  ...
  const [sharingCode, setSharingCode] = useState(
    props.route?.params?.sharingCode ?? null
  );
  ...
  const onTermsAccepted = async () => {
    try {
      ...
      if (!result.success) {
        ...
        //TODO: colocar status active el usuario.
      }
      setLoading(false);
    } catch (error) {
        ...
        <View style={styles.mainContainer}>
          ...
          <CustomInput
            ...
            onChangeText={(username) => {
              username.length <= 15
                ...
                : setError(translate("authentication.usernameLengthErrorMax"));
            }}
            returnKeyType="next"
            blurOnSubmit={false}
            autoCapitalize="none"
          ...
          <RNPickerSelect
            ...
            onValueChange={(value, index) => {
              setCountry(countryItems[index - 1] ?? null);
            }}
            items={countryItems}
          >
            <CustomInput
          ...
          {!!(password && password !== repeatPassword) && (
            <Text style={styles.errorText}>
              {translate("authentication.passwordsDoNotMatch")}
            </Text>
          )}
          <View style={{ paddingVertical: 10 }} />
          <CustomButton
            loading={loading}
            title={translate("authentication.create")}
            inverse
            disabled={!filledSignup}
            onPress={signUp1}
          />
          <View style={{ paddingVertical: 10 }} />
          ...
          <Modal
            overlayStyle={{ height: "90%", padding: 5 }}
